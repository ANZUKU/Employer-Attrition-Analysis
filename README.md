...
TotalEmployees = COUNT(HR_Data[EmployeeNumber])

Attrited Employees = CALCULATE(COUNT(HR_Data[Attrition]), HR_Data[Attrition] = "Yes")

Active Employees = CALCULATE(COUNT(HR_Data[Attrition]), HR_Data[Attrition] = "No")

Attrition Rate = DIVIDE([Attrited Employees], [Total Employees], 0)

Average Salary = AVERAGE(HR_Data[MonthlyIncome])

Average Years at Company = AVERAGE(HR_Data[YearsAtCompany])

Attrition Rate by Department = DIVIDE(
    CALCULATE(COUNT(HR_Data[Attrition]), HR_Data[Attrition] = "Yes"),
    CALCULATE(COUNT(HR_Data[Attrition])),
    0
)

Average Job Satisfaction = AVERAGE(HR_Data[JobSatisfaction])

Average Environment Satisfaction = AVERAGE(HR_Data[EnvironmentSatisfaction])

Average Relationship Satisfaction = AVERAGE(HR_Data[RelationshipSatisfaction])
